...
FRED_TIMESTAMP_PATTERN = re.compile(
    r"^\d{4}-(0[1-9]|1[0-2])-\d{2} \d{2}:\d{2}:\d{2}[+-](0\d|1[0-4])$"  # offset 00–14 covers all real UTC offsets
)


def validate_fred_timestamp(s: str) -> str:
    s = s.strip()
    if len(s) == 0:
        raise ValueError("Value cannot be empty")
    if not FRED_TIMESTAMP_PATTERN.match(s):
        raise ValueError(
...
FredTimestamp = Annotated[str, AfterValidator(validate_fred_timestamp)]
