...
def test_rejects_missing_api_key() -> None:
    with pytest.raises(ValidationError):
        category_children.RequestParams(  # type: ignore[call-arg]
            file_type=category_children.FileType.json,
            category_id=0,
        )
...
def test_rejects_negative_category_id() -> None:
    with pytest.raises(ValidationError):
        category_children.RequestParams(
            api_key=_VALID_API_KEY,
            file_type=category_children.FileType.json,
            category_id=-1,
        )
