...
from fred import category_children


@pytest.mark.contract_test
def test_accepts_single_category() -> None:
    r = category_children.Response(
        categories=[{"id": 0, "name": "Categories", "parent_id": 0}]  # ty: ignore[invalid-argument-type]
    )
...
def test_accepts_empty_list() -> None:
    r = category_children.Response(categories=[])
    assert r.categories == []
...
def test_rejects_category_with_negative_id() -> None:
    with pytest.raises(ValidationError):
        category_children.Response(
            categories=[{"id": -1, "name": "Bad", "parent_id": 0}]  # ty: ignore[invalid-argument-type]
        )
...
def test_rejects_missing_categories_field() -> None:
    with pytest.raises(ValidationError):
        category_children.Response()  # ty: ignore[missing-argument]
