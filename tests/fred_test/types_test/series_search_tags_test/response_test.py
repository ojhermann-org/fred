...
from fred import series_search_tags
...
def test_accepts_valid_response() -> None:
    r = series_search_tags.Response.model_validate(_valid_response())
...
def test_accepts_empty_tags() -> None:
    r = series_search_tags.Response.model_validate(_valid_response(tags=[], count=0))
    assert r.tags == []
...
def test_rejects_invalid_order_by() -> None:
    with pytest.raises(ValidationError):
        series_search_tags.Response.model_validate(_valid_response(order_by="invalid"))
...
def test_rejects_missing_tags() -> None:
    with pytest.raises(ValidationError):
        ...
        del data["tags"]
        series_search_tags.Response.model_validate(data)
