...
from fred import release_related_tags
...
def test_accepts_valid_response() -> None:
    r = release_related_tags.Response.model_validate(_valid_response())
...
def test_accepts_multiple_tags() -> None:
    r = release_related_tags.Response.model_validate(
        _valid_response(
...
def test_accepts_empty_tags_list() -> None:
    r = release_related_tags.Response.model_validate(_valid_response(tags=[], count=0))
    assert r.tags == []
...
def test_rejects_invalid_order_by() -> None:
    with pytest.raises(ValidationError):
        release_related_tags.Response.model_validate(
...
def test_rejects_missing_tags_field() -> None:
    with pytest.raises(ValidationError):
        ...
        del data["tags"]
        release_related_tags.Response.model_validate(data)
