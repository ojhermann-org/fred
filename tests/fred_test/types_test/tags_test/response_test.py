...
def test_accepts_valid_response() -> None:
    r = tags.Response.model_validate(_valid_response())
    assert r.order_by == OrderBy.series_count
    assert r.sort_order == SortOrder.asc
    assert len(r.tags) == 1
    assert r.tags[0].name == "monetary+aggregates"


@pytest.mark.contract_test
def test_accepts_null_notes() -> None:
    r = tags.Response.model_validate(_valid_response(tags=[_valid_tag(notes=None)]))
    assert r.tags[0].notes is None


@pytest.mark.contract_test
def test_accepts_empty_tags() -> None:
    r = tags.Response.model_validate(_valid_response(tags=[], count=0))
    assert r.tags == []
...
def test_rejects_invalid_order_by() -> None:
    with pytest.raises(ValidationError):
        tags.Response.model_validate(_valid_response(order_by="invalid"))
...
def test_rejects_missing_tags() -> None:
    with pytest.raises(ValidationError):
        ...
        del data["tags"]
        tags.Response.model_validate(data)


@pytest.mark.contract_test
def test_rejects_invalid_realtime_start() -> None:
    with pytest.raises(ValidationError):
        tags.Response.model_validate(_valid_response(realtime_start="not-a-date"))
